...
**할 일 — mTLS 인증서 재발급.** 2026-09-18 반납 직전에 개인키를 대화창에
붙여넣어 기록에 남았다. 콘솔에서 새로 발급받아 교체할 것:
...
급하지 않다 — 이 인증서로 할 수 있는 건 입찰레이더 사용자에게 푸시를 보내는 것뿐이고
`PUSH_SEND`도 아직 꺼져 있다. 하지만 **푸시를 켜기 전에는** 끝내 둘 것.
...
> ⚠️ **번들 만들기 전에 광고 키부터 바꿀 것.**
> `miniapp/.env`의 `VITE_AD_GROUP_ID`가 아직 `ait-ad-test-banner-id`다.
> 테스트 키가 남으면 비게임 **전체 점검에서 반려**된다.
> 콘솔 → 수익화 → 광고에서 운영 광고그룹 ID를 받아 `.env`에 넣고 다시 빌드한다.
