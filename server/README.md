...
- **나라장터 조회 구간도 KST다.** SQL만 고치면 끝이 아니다 — `inqryBgnDt`/`inqryEndDt`에 naive `datetime.now()`를 넣으면 컨테이너(UTC)에서 9시간 전 구간을 조회해 **공고가 9시간씩 늦게 들어온다**. 에러 없이 조용히 늦는다. `g2b.now_kst()`를 쓰고, 테스트가 AST로 naive 호출을 막는다.
