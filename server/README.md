...
- **타임존을 KST로 못 박는다.** 컨테이너는 UTC로 도는데 공고 시각은 전부 KST다. 그대로 두면 `datetime('now','localtime')`이 9시간 이르게 나와서 **이미 마감된 공고를 계속 알린다.** `api.py`와 `g2b.py`가 import 시점에 `TZ=Asia/Seoul`을 강제하고, 테스트가 이를 고정한다.
